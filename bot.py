...
def start(msg):
    ...
    if user_exists(user_id):
        if is_logged_in(user_id):
            ...
            bot.send_message(msg.chat.id, f'👋 Снова здесь, {name}!', reply_markup=main_keyboard)
        else:
...
def process_slots(msg):
    ...
    if win > 0:
        trophies_gain = 10
        update_user(user_id, trophies=user.get('trophies', 0) + trophies_gain)
        text = f'🎉 **ВЫИГРЫШ!**\n{result[0]} {result[1]} {result[2]}\n💰 +{win} монет\n🏆 +{trophies_gain} трофеев'
    else:
        trophies_loss = -5
        new_trophies = max(0, user.get('trophies', 0) + trophies_loss)
...
def process_change_trophies(msg):
    ...
    try:
        ...
        new_trophies = max(0, user.get('trophies', 0) + amount)
        update_user(user['id'], trophies=new_trophies)
        
        text = f'✅ Трофеи "{name}" изменены: {user.get("trophies", 0)} → {new_trophies}'
        if amount > 0:
            text += f' (+{amount})'
        else:
            text += f' ({amount})'
