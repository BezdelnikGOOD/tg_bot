...
creator_keyboard.add('🔊 Размутить', '⬅️ Назад')
...
def save_chat_message(user_id, nick, message):
    ...
    try:
        ...
        cur.execute('''
            ...
            VALUES (%s, %s, %s, %s)
        ''', (user_id, nick, message, datetime.now().strftime('%H:%M')))
        conn.commit()
        cur.close()
    finally:
        release_db_connection(conn)
...
# ============================================================
# КЕЙСЫ (С ФОТО И ИСПРАВЛЕННЫМИ ОБРАБОТЧИКАМИ)
# ============================================================
...
'''
    ...
    # Используем edit_message_caption для фото
...
def process_buy_case_logic(msg, user_id, case_id, case, user):
    """Логика открытия кейса с фото в финале"""
...
def change_boss_time(msg):
    ...
    try:
        ...
        new_end = (datetime.now() + timedelta(minutes=minutes)).isoformat()
